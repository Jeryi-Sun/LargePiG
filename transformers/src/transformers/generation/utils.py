...
class GenerationMixin:
    ...
    def generate(
        ...
        only_pointer: Optional[bool] = None,
        start_p=None, end_p=None, ref_prompt=None, 
        scale_value=100, normalization=False, 
        hallucination_detect=False, use_entropy=False,
        alpha=0.5, attention_layers=None,
        **kwargs,
        ...
        if generation_mode == GenerationMode.GREEDY_SEARCH and largepig_decoding:
            if generation_config.num_return_sequences > 1:
                raise ValueError(
            ...
            return self.largepig_greedy_decode(
                input_ids,
                logits_processor=prepared_logits_processor,
                stopping_criteria=prepared_stopping_criteria,
                pad_token_id=generation_config.pad_token_id,
                ...
                streamer=streamer,
                pointer=pointer,
                only_pointer=only_pointer,
                prefix_length=input_ids.shape[-1],
                start_p=start_p, end_p=end_p, 
                ref_prompt=ref_prompt, scale_value=scale_value, normalization=normalization, 
                hallucination_detect=hallucination_detect, use_entropy=use_entropy, alpha=alpha,
                **model_kwargs,
            )
        elif generation_mode == GenerationMode.GREEDY_SEARCH and contrastive_decoding:
            if generation_config.num_return_sequences > 1:
                raise ValueError(
                    f"num_return_sequences has to be 1, but is {generation_config.num_return_sequences} when doing"
                    " greedy search."
        ...
        elif generation_mode == GenerationMode.SAMPLE and largepig_decoding:
        ...
        elif generation_mode == GenerationMode.SAMPLE and contrastive_decoding:
            # 11. prepare logits warper
            logits_warper = self._get_logits_warper(generation_config)
    ...
    def calculate_hallucination_mask(self, input_ids, words_logits, start_p, end_p, attentions, use_entropy, alpha=0.5):
        threshold = np.log(self.config.vocab_size)+self.config.vocab_size if use_entropy else np.log(self.config.vocab_size)
        shift_logits = words_logits[:, start_p:end_p-1, :]
        shift_labels = input_ids[:, start_p+1:end_p]
        shift_attentions = attentions[:, start_p:end_p-1, start_p:end_p-1] 
        ...
        hc_his = torch.bmm(shift_attentions, hc.unsqueeze(2)).squeeze(2)
        hc_final = alpha*(hc_his + hc * (1 - attentions_sum)) + (1-alpha)*hc
        hallucination_mask = (hc_final <= threshold).float()
        batch_size, seq_len_minus_one = hallucination_mask.shape
        # Create a tensor of ones with shape (batch_size, 1)
        ones_to_concat = torch.ones((batch_size, 1), dtype=hallucination_mask.dtype, device=hallucination_mask.device)
    ...
    def largepig_greedy_decode(
        ...
        only_pointer: Optional[bool] = False,
        ...
        **model_kwargs,
        ...
        while True:
            ...
            if base_layer is not None:
                ...
                if hallucination_detect:
                    if start_p != None and end_p != None:
                        ...
                        start_p = prefix_length-ref_prompt.shape[-1]
                        end_p = prefix_length
                        hallucination_mask = self.calculate_hallucination_mask(input_ids, dict_outputs[mature_layer], start_p, end_p, attentions, use_entropy=use_entropy, alpha=alpha)
                ...
                pointer_scores = attentions[:, -1, :]  # shape: (batch_size, sequence_length)
                ...
                if start_p != None and  end_p != None:
                    ...
                    pointer_probs =  pointer_scores[:,prefix_length-ref_prompt.shape[-1]:prefix_length]
                    input_ids_cp =  input_ids[:,prefix_length-ref_prompt.shape[-1]:prefix_length]
                else:
                    pointer_probs =  pointer_scores[:,:prefix_length]  # shape: (batch_size, prefix_sequence_length) todo: 截取这一步还需要操作一下只让模型关注文本内容
                    input_ids_cp = input_ids[:,:prefix_length] 
                ...
                if self.config.pad_token_id is None:
                    special_tokens_mask = input_ids_cp.eq(self.config.bos_token_id) | input_ids_cp.eq(self.config.eos_token_id)
                else:
                    special_tokens_mask = input_ids_cp.eq(self.config.pad_token_id) | input_ids_cp.eq(self.config.bos_token_id) | input_ids_cp.eq(self.config.eos_token_id)
                ...
                # Note: 'input_ids' here is assumed to be the context tokens that we are pointing to
            ...
            # add pointer distribution
            if pointer:
                p_cp = torch.clamp(p_cp*scale_value, min=0, max=0.5)
                weight = (1-p_cp.unsqueeze(1))
                probs = p_cp.unsqueeze(1) * pointer_vocab_distr +  weight * probs
    ...
    def largepig_sample(
        ...
        prefix_length: Optional[int] = None,
        start_p=None, end_p=None, ref_prompt=None, scale_value=100, 
        normalization=False, hallucination_detect=False, use_entropy=False, alpha=0.5,
        **model_kwargs,
