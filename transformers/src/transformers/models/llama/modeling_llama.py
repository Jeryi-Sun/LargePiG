        ...
        if early_exit_layers is not None:
            ...
            for i, early_exit_layer in enumerate(early_exit_layers):
                if self.config.pretraining_tp > 1:
                    ...
                    logits = [F.linear(outputs.hidden_states[early_exit_layer], lm_head_slices[i]) for i in range(self.config.pretraining_tp)]
                    logits = torch.cat(logits, dim=-1)
                else:
                    logits = self.lm_head(outputs.hidden_states[early_exit_layer])
                logits_dict[early_exit_layer] = logits

            # if early_exit_layer miss last layer, add it!
            ...
            logits = logits.float()
            logits_dict[self.config.num_hidden_layers] = logits
